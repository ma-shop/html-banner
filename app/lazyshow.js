...
export default function LazyShow(options = {}) {
  ...
  this.options = {
    ...
    added: 'lazy.added',
    callback: null, // if you don't want to use an event listener then you can just run a callback
  ...
  this.container = this.has_tag ? this.options.container : window

  // defines out the initial nodes list when the function is called.
  this.nodes = [].slice.call(this.container.querySelectorAll ? this.container.querySelectorAll(this.options.selector) : document.querySelectorAll(this.options.selector)) // eslint-disable-line
  ...
  if (this.has_tag) {
    this.check = () => {
      this.last_y = this.container.scrollTop + this.getOffset(this.container)
      this.last_x = this.container.scrollLeft + this.getOffset(this.container, 'left')
      return this.tick()
    }
  } else {
    this.check = () => {
...
LazyShow.prototype = {
  ...
  /// @arg {string} event - The name of the event to create
  /// @arg {boolean} bubbles - If true then the event will be set to bubble up
  createEvent(event, bubbles = true) {
    let custom_event
    if (window.CustomEvent) { // Browser is normal
      custom_event = new CustomEvent(event, { bubbles })
    } else { // Browser sucks and is probably IE
      console.log('Come on IE, really bro?')
      custom_event = document.createEvent('CustomEvent')
      custom_event.initCustomEvent(event, true, true, { bubbles })
    }
  ...
  tick() {
    if (!this.ticking) {
      requestAnimationFrame(this::this.checkNodes)
      this.ticking = true
    }
    return this
  },
  ...
  listeners(type = 'add') {
    ...
    for (const event of [ 'resize', 'orientationchange', 'hashchange' ]) {
      window[type](event, this.throttle(this.check, 200), false)
    }

    // add/remove event's that can be added to container
    for (const event of [ 'scroll', this.options.check ]) {
      this.container[type](event, this.check, false)
    }
    ...
    if (!!this.options.callback) {
      ...
      this.container[type](this.options.namespace, this.options.callback)
    }
  },

  on(event, callback) {
    this.container.addEventListener(this.options[event] || event, (e) => callback.call(e.target, e))
    return this
  },
  ...
  emit(event, target) {
    ...
    (target || this.container).dispatchEvent(event)
    return this
  },
  ...
  isVisible(node, threshold) {
    ...
    return (
      this.isVisibleVertical(node, threshold) &&
      this.isVisibleHorizontal(node, threshold)
    )
  },
  ...
  checkParents(node, callback) {
    function matches(el, selector) {
      const match = el && (el.matches || el.webkitMatchesSelector || el.mozMatchesSelector || el.msMatchesSelector || el.oMatchesSelector)
    ...
    if (typeof callback === 'function') {
      while (
        ...
        node !== this.container.parentElement &&
  ...
  checkNodes() {
    const not_visible = [] // stores nodes that still aren't visible
    const nodes_length = this.nodes.length

    // set the container height so it can be used in `isVisible`
    this.container_height = this.container.innerHeight || this.container.offsetHeight
    this.container_width = this.container.innerWidth || this.container.offsetWidth

    // a) loop over each node in the list and check if they're visible
    if ((this.has_tag ? !this.isHidden(this.container) : true) && nodes_length) {
  ...
  observer() {
    ...
    return Observer && new Observer((mutations) => {
      ...
      for (const mutation of mutations) {
        ...
        for (const added_node of mutation.addedNodes) {
          ...
          if (added_node.querySelectorAll) {
            children_nodes = [].slice.call(added_node.querySelectorAll(this.options.selector))
          }
      ...
      .observe(this.container.nodeType === 1 ? this.container : document.documentElement, {
        childList: true,
        characterData: true,
        subtree: true,
      })
